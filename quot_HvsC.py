...
class Game:
    ...
    def BattleStep1_2(self,count_card,name,card,field):#H用
        Incard_temp = 0
        a = input("何枚引きますか？:")
        draw = int(a)
        if count_card + draw <= 5 and draw != 0:
            print("必要なカードが左から何番目の数字かを入力してください")
            count_card = count_card + draw
            while draw != 0:
                while True:
                    a = input()
                    Incard = int(a)
                    if (Incard==1 or Incard==2 or Incard==3) and Incard != Incard_temp:
                        break
                    print("1から3までの数字で入力してください(同じ数字を打つことはできません)")
                Incard_temp = Incard
                Incard = Incard - 1
                card.append(field[Incard])#場から入れるカードを手札に代入
                field[Incard] = self.deck.pop()#山札から場のカードへ追加及び、手札に入れる場のカードを削除
                draw -= 1
            return count_card,card,field
        else:
            print("引かない、もしくは手札の枚数が5枚を超えることはできません")
            return self.BattleStep1_2(count_card,name,card,field)

    def choice1(self,hand,count):#H用
        dust = 0
        print("必要ないカードが手札の左から何番目かを数字で入力してください")
        lengh = len(hand)
        while True:
            a = input()
            Outcard = int(a)
            if Outcard >= 1 and Outcard <= lengh:
                break
            print("持っている枚数の数字で入力してください")
        Outcard = Outcard - 1
        dust = hand[Outcard]
        del hand[Outcard]
        count = count - 1
        return hand,count,dust

    def choice2(self,hand,count,conf_card,conf_result,conf_resultN):#H,C兼用
    ...
    def pattern(self,hand,field): #仮想手札に場からカードを代入、一番優先順位が高いものの手札を調べる
        high_choice = 0 #優先順位の値
        high_dust = 0 #優先順位が高いものの捨て札
    ...
    def play_game(self):
        ...
        #cpu1
            ...
            count1_card,p1h,field_card = self.BattleStep1_2(count1_card,p1n,p1h,field_card)
            ...
            print("手札から必要ないと判断したカード1枚を捨てる : 1")
            print("5枚1セットの役として確定させる : 2")
            while True:
                choice = input(">")
                choice = int(choice)
                if choice==1:
                    p1h,count1_card,result_dust = self.choice1(p1h,count1_card)
                    dust_card.append(result_dust)
                    break
                    ...
                    print("カードが5枚ないときに2を選択することと1,2以外の数字は使えません")
            self.alloutput(p1n,p1h,count1_card,conf_name1,conf_card1,p2n,p2h,count2_card,conf_name2,conf_card2,field_name,field_card,dust_name,dust_card)
            time.sleep(1)
            if len(self.deck.deck) == 0:
                break
